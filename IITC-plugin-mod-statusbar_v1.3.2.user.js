...
// @description    [1.3.2]ステータスバーに、装着MODの略号を色付き・スロット位置固定で表示する。挿入位置は「ポータルレベル/XMエネルギーの前」と「その後・ポータル名の前」から選択可能。画面下情報バーの左パディングを1px単位で調整でき (既定5px)、画面端が丸いスマホでMOD文字が隠れる問題に対応。空欄文字・MODアイコン・MODカラーを一括初期化するボタン付き。設定ダイアログの入力欄では、Androidで文字カーソルが端に達した際に矢印キーで隣の要素へフォーカスが移るブラウザ既定動作 (空間ナビゲーション) を抑止し、カーソル移動を自前処理。さらにタップ/Tab以外でフォーカスが外れた場合は直後に強制復帰させ、文字入力がキャンセルされる不具合に対応。設定画面はOKボタン1つで反映・閉じるを行う。IITC.statusbar API の無い旧ビルド (iOS版アプリ等) にもフォールバック対応。
...
function wrapper(plugin_info) {
    ...
    self.showSettings = function () {
        ...
        // --- フォーカス飛びの根本対策 (原因はAndroid/Chromiumの仕様) ------
        // Androidのオンスクリーンキーボードの矢印キーはDPADキーイベントとして
        // 届き、文字カーソルが入力欄の先頭/末尾にある状態でさらに左/右を
        // 押すと、Chromium/WebViewの既定動作 (空間ナビゲーション) として
        // 隣のフォーカス可能要素へフォーカスが移ってしまう。略号は2〜4文字と
        // 短くカーソルがすぐ端に達するため、この既定動作が頻発していた。
        // これはブラウザ仕様だが、キーイベントがJSに届く限り preventDefault
        // で打ち消せる。矢印キーによるカーソル移動を自前で行い、既定動作
        // (端に達した時のフォーカス移動を含む) を常に抑止する。
        // ※ IME変換中 (isComposing / keyCode 229) の矢印キーは変換操作の
        //   ため触らない。修飾キー付きも通常操作を尊重して触らない。
        dlg.on('keydown', 'input[type="text"]', function (e) {
            ...
            e.preventDefault(); // 空間ナビゲーションによるフォーカス移動を抑止
            try { input.setSelectionRange(pos, pos); } catch (err) { /* 念のため */ }
        });

        // --- フォーカス強制復帰 (事後救済の安全網) -------------------------
        // キーイベントがJSに届かない経路 (IMEがカーソル移動を内部処理する
        // 機種・キーボード等) はAndroidの仕様上、事前に抑止できない。
        // その場合は「タップ/クリックの直後」と「Tabキー移動」だけを正当な
        // フォーカス移動として許可し、それ以外の要因でフォーカスが外れたら
        // 直後に元の入力欄へカーソル位置ごと強制復帰させる。
        // ※ v1.3.1では、入力欄を最初にタップした時の記録が使用後も残り続け、
        //   以後のフォーカス飛びが全て「タップによる正当な移動」と誤判定
        //   されて復帰処理が一度も動かないバグがあった (今回修正)。
        //   タップ記録はタイムスタンプで管理し、blur直前の短時間内に
        //   発生したものだけを有効とする。
        // ※ タップの監視はdocumentのキャプチャ段階で行う。OKボタンや
        //   タイトルバーはダイアログ内容部の外にあり、地図タップは他の
        //   ハンドラに伝播を止められる可能性があるため、どこへのタップも
        //   確実に検知できる位置で拾う必要がある。
        var lastPointerTime = 0;
        var tabNavigating = false;
        var onAnyPointer = function () { lastPointerTime = Date.now(); };
        ...
        var unbindPointerTracking = function () {
        ...
        dlg.on('dialogclose remove', unbindPointerTracking);

        dlg.on('keydown', 'input', function (e) {
            tabNavigating = (e.key === 'Tab' || e.keyCode === 9);
        });
        dlg.on('blur', 'input', function () {
            var input = this;
            var selStart = null, selEnd = null;
            try {
                selStart = input.selectionStart;
                selEnd = input.selectionEnd;
            } catch (err) { /* type=color/number等はselection非対応 */ }
            // 直近0.7秒以内にどこかへのタップ/クリックがあれば、それに伴う
            // 正当なフォーカス移動 (別の入力欄・OKボタン・地図など) とみなす
            var byTap = (Date.now() - lastPointerTime) < 700;
            var byTab = tabNavigating;
            tabNavigating = false;
            if (byTap || byTab) return;
            // 同期的なblur直後はまだ新しいフォーカス先が確定していないため、
            // 1tick後に確認してから戻す。タップ以外での移動は、移動先が
            // ダイアログ内の別の入力欄であっても許可しない (それがまさに
            // 報告されている「別の要素に移動してしまう」不具合のため)。
            setTimeout(function () {
                ...
                } catch (err) { /* type=color/number等は非対応 */ }
            }, 0);
        });
