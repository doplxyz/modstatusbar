...
// @description    [1.3.3]ステータスバーに、装着MODの略号を色付き・スロット位置固定で表示する。挿入位置は「ポータルレベル/XMエネルギーの前」と「その後・ポータル名の前」から選択可能。画面下情報バーの左パディングを1px単位で調整でき (既定5px)、画面端が丸いスマホでMOD文字が隠れる問題に対応。空欄文字・MODアイコン・MODカラーを一括初期化するボタン付き。設定ダイアログの入力欄はタップ/Tab以外でフォーカスが移らないよう保護: 矢印キーで文字カーソルが端に達した際に隣の要素へフォーカスが移るブラウザ既定動作 (空間ナビゲーション) をdocumentキャプチャ段階の横取りで抑止し、それでも防げない経路はガード対象を一元管理する監視でカーソル位置ごと即時強制復帰させる (実Chromiumの空間ナビゲーション有効環境での自動テストで検証済み)。設定画面はOKボタン1つで反映・閉じるを行う。IITC.statusbar API の無い旧ビルド (iOS版アプリ等) にもフォールバック対応。
...
function wrapper(plugin_info) {
    ...
    self.showSettings = function () {
        ...
        // ==== フォーカスガード ================================================
        // 症状: Androidで略号入力中に文字カーソルを左右移動させると、カーソルが
        // 入力欄の端 (先頭/末尾) に達した時点で隣の要素へフォーカスが移り、
        // 入力がキャンセルされる。
        // 原因: オンスクリーンキーボードの矢印キーはDPADキーイベントとして届き、
        // カーソルが端にある状態の左/右はChromium/WebViewの既定動作
        // 「空間ナビゲーション」としてフォーカス移動になる (Android/ブラウザ側の
        // 仕様。デスクトップChromiumの --enable-spatial-navigation で同一症状の
        // 再現を確認済み)。
        // 対策は二段構え:
        //  (1) キーイベントがJSに届く経路: documentキャプチャ段階で横取りし、
        //      矢印キーのカーソル移動を自前処理して既定動作 (フォーカス移動と
        //      Leaflet地図のパン/ズーム) を完全に抑止する。
        //  (2) キーイベントがJSに届かない経路 (IME内部のカーソル操作等。
        //      ここだけはAndroidの仕様上、事前抑止が不可能):
        //      「ガード対象の入力欄」をダイアログ全体で1つだけ一元管理し、
        //      タップ/Tab以外の要因でフォーカスが他所へ移ったら即座に
        //      カーソル位置ごと強制復帰させる。
        // ※ v1.3.2までは入力欄ごとのblurハンドラで復帰していたため、移動先が
        //   同じダイアログ内の別の入力欄だった場合に両者の復帰処理が相互発火
        //   して綱引きになり、フォーカスが不定な要素で止まる不具合があった
        //   (「ヘンな所に飛ぶ」報告の正体)。一元管理方式でこれを解消した。

        var guardedInput = null;   // いまガードすべき入力欄 (常に高々1つ)
        var guardedSel = null;     // その入力欄の直近のカーソル位置 [start, end]
        var tabNavigating = false; // 直前のkeydownがTabだったか
        var lastPointer = { t: 0, target: null }; // 直近のタップ/クリック

        var isDlgInput = function (el) {
            return !!(el && el.nodeName === 'INPUT' && dlg[0] && $.contains(dlg[0], el));
        };
        // そのタップが要素elへのフォーカス移動を正当化するか
        // (el自身またはその内側を直近0.7秒以内にタップしている場合のみ)。
        // オンスクリーンキーボードは別サーフェスでページにタップイベントを
        // 発生させないため、キー操作起因の移動がこれで許可されることはない。
        var tappedOn = function (el) {
            if (!el || !lastPointer.target) return false;
            if (Date.now() - lastPointer.t >= 700) return false;
            return el === lastPointer.target || el.contains(lastPointer.target);
        };
        var restoreFocus = function () {
            if (!guardedInput) return;
            if (!dlg.is(':visible')) return; // OK等で閉じた後は何もしない
            if (document.activeElement === guardedInput) return;
            try { guardedInput.focus(); } catch (err) { return; }
            if (guardedSel && guardedInput.setSelectionRange) {
                try { guardedInput.setSelectionRange(guardedSel[0], guardedSel[1]); } catch (err) { /* 非対応type */ }
            }
        };
        // Android IMEはフォーカス処理が非同期に走ることがあるため、
        // 直後と少し後の2回確認する (restoreFocusは冪等)。
        var scheduleRestore = function () {
            setTimeout(restoreFocus, 0);
            setTimeout(restoreFocus, 120);
        };

        // --- (1) キー横取り: documentのキャプチャ段階 -----------------------
        // ダイアログ内の入力欄が対象のキーイベントを誰よりも先に受け取り、
        // Leaflet (documentのkeydownで矢印パン/±ズーム) やその他のハンドラへ
        // 一切渡さない。文字入力・カーソル移動などのブラウザ既定動作は
        // stopPropagationでは失われない。
        var onKeyCapture = function (e) {
            var input = e.target;
            if (!isDlgInput(input)) return;
            var key = e.key || '';
            var code = e.keyCode || e.which || 0;
            if (e.type === 'keydown') tabNavigating = (key === 'Tab' || code === 9);
            // ESCはダイアログを閉じるjQuery UIの既定機能に、Tabはブラウザの
            // フォーカス移動に、それぞれ渡す必要があるため素通しする。
            if (key === 'Escape' || code === 27 || key === 'Tab' || code === 9) return;
            if (e.stopImmediatePropagation) e.stopImmediatePropagation();
            else e.stopPropagation();
            if (e.type !== 'keydown') return;
            // IME変換中の矢印キーは文節移動などの変換操作なので触らない
            if (e.isComposing || code === 229) return;
            if (e.altKey || e.ctrlKey || e.metaKey || e.shiftKey) return;
            if (input.type !== 'text') return; // number/color/checkbox は既定動作を尊重
            ...
            if (key === 'ArrowLeft' || code === 37) {
                pos = (start === end) ? Math.max(0, start - 1) : start;
            } else if (key === 'ArrowRight' || code === 39) {
                pos = (start === end) ? Math.min(len, end + 1) : end;
            } else if (key === 'ArrowUp' || code === 38) {
                pos = 0;   // 単一行入力での慣例的挙動 (先頭へ)
            } else if (key === 'ArrowDown' || code === 40) {
                pos = len; // 同上 (末尾へ)
            } else {
                return;
            }
            e.preventDefault(); // 端に達した際の空間ナビゲーション(フォーカス移動)を抑止
            try { input.setSelectionRange(pos, pos); } catch (err) { /* 念のため */ }
        };

        // --- (2) フォーカス移動の監視と強制復帰 -----------------------------
        var onAnyPointer = function (e) {
        ...
        var onFocusIn = function (e) {
            ...
            var byTab = tabNavigating;
            if (isDlgInput(t)) {
                // ガード対象入力欄へのフォーカス。自分たちの復帰処理による
                // 再フォーカス (t === guardedInput) と、タップ/Tabによる正当な
                // 移動はそのまま受け入れてガード対象を切り替える。
                if (t === guardedInput || byTab || tappedOn(t) || !guardedInput) {
                    guardedInput = t;
                    if (byTab) tabNavigating = false;
                    return;
                }
                // タップ以外で別の入力欄へ移された → 不正な移動として復帰
                scheduleRestore();
                return;
            }
            // 入力欄以外 (OKボタン・タイトルバー・地図等) へのフォーカス
            if (byTab || tappedOn(t)) {
                // 正当な移動: ガードを解いてユーザーの操作に従う
                guardedInput = null;
                if (byTab) tabNavigating = false;
                return;
            }
            if (guardedInput) scheduleRestore(); // 不正な移動 → 復帰
        };
        var onFocusOut = function (e) {
            if (e.target !== guardedInput || !guardedInput) return;
            // 復帰に備えて直近のカーソル位置を保存 (focusout時点ではまだ読める)
            try {
                guardedSel = [guardedInput.selectionStart, guardedInput.selectionEnd];
            } catch (err) {
                guardedSel = null; // type=color/number等はselection非対応
            }
            // フォーカスがどの要素にも移らない場合 (bodyへ落ちる場合) は
            // focusinが発火しないため、こちらでも確認する。
            setTimeout(function () {
                if (!guardedInput || !dlg.is(':visible')) return;
                var active = document.activeElement;
                if (active && active !== document.body && active !== document.documentElement) return; // 移動先ありはfocusin側が判定済み
                if ((Date.now() - lastPointer.t) < 700) {
                    // 地図などフォーカスを持たない場所へのタップ: 正当な離脱
                    guardedInput = null;
                    return;
                }
                restoreFocus();
        ...
        // 監視はdocumentのキャプチャ段階で行う。OKボタンやタイトルバーは
        // ダイアログ内容部の外にあり、地図タップは他のハンドラに伝播を
        // 止められる可能性があるため、確実に拾える位置で監視する必要がある。
        ...
        document.addEventListener('touchstart', onAnyPointer, true);
        document.addEventListener('mousedown', onAnyPointer, true);
        document.addEventListener('focusin', onFocusIn, true);
        document.addEventListener('focusout', onFocusOut, true);
        var unbindFocusGuard = function () {
            guardedInput = null;
            document.removeEventListener('keydown', onKeyCapture, true);
            document.removeEventListener('keyup', onKeyCapture, true);
            document.removeEventListener('keypress', onKeyCapture, true);
            document.removeEventListener('pointerdown', onAnyPointer, true);
            document.removeEventListener('touchstart', onAnyPointer, true);
            document.removeEventListener('mousedown', onAnyPointer, true);
            document.removeEventListener('focusin', onFocusIn, true);
            document.removeEventListener('focusout', onFocusOut, true);
        };
        // ダイアログが閉じられたらdocumentの監視を必ず解除する
        dlg.on('dialogclose remove', unbindFocusGuard);
