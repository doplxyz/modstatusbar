...
// @description    [1.3.0]ステータスバーに、装着MODの略号を色付き・スロット位置固定で表示する。挿入位置は「ポータルレベル/XMエネルギーの前」と「その後・ポータル名の前」から選択可能。画面下情報バーの左パディングを1px単位で調整でき (既定5px)、画面端が丸いスマホでMOD文字が隠れる問題に対応。空欄文字・MODアイコン・MODカラーを一括初期化するボタン付き。略号入力欄は半角4文字分の幅を確保し、Androidで入力中に矢印キーでフォーカスが飛ぶ不具合にも対応。設定画面はOKボタン1つで反映・閉じるを行う。IITC.statusbar API の無い旧ビルド (iOS版アプリ等) にもフォールバック対応。
...
function wrapper(plugin_info) {
    ...
    self.showSettings = function () {
        ...
        // カーソル左右移動 (矢印キー相当のkeydown) を行うと、そのイベントが
        // documentまで伝播し、Leaflet地図のキーボード操作ハンドラ
        // (L.Map.Keyboard._onKeyDown。地図が一度でもフォーカスされていると
        // documentへ直接bindされ、フォーカス中の要素がテキスト入力かどうかを
        // 判定せずに矢印キーで地図をパンしてしまう) に反応してしまい、
        // 入力中の要素からフォーカスが外れて文字入力がキャンセルされる。
        // タップ以外でフォーカスが移らないよう、ダイアログ内の入力要素の
