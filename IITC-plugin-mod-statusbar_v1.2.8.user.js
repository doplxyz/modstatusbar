...
// @description    [1.2.8]ステータスバーに、装着MODの略号を色付き・スロット位置固定で表示する。挿入位置は「ポータルレベル/XMエネルギーの前」と「その後・ポータル名の前」から選択可能。画面下情報バーの左パディングを1px単位で調整でき、画面端が丸いスマホでMOD文字が隠れる問題に対応。ON/OFFトグル、略号・色・空欄文字のユーザーカスタマイズに対応。IITC.statusbar API の無い旧ビルド (iOS版アプリ等) にもフォールバック対応。設定画面はOKボタン1つで反映・閉じるを行う。
...
function wrapper(plugin_info) {
    ...
    self.prepend = true;
    // 画面下の情報バー (#mobileinfo) の左パディング追加分 (px単位、既定0)。
    ...
    self.loadSettings = function () {
        ...
        self.offsetX = parseInt(localStorage[self.KEY_OFFSETX], 10);
        if (!isFinite(self.offsetX) || self.offsetX < 0) self.offsetX = 0;
        try {
