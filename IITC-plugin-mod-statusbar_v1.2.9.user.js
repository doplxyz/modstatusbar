...
// @description    [1.2.9]ステータスバーに、装着MODの略号を色付き・スロット位置固定で表示する。挿入位置は「ポータルレベル/XMエネルギーの前」と「その後・ポータル名の前」から選択可能。画面下情報バーの左パディングを1px単位で調整でき (既定5px)、画面端が丸いスマホでMOD文字が隠れる問題に対応。空欄文字・MODアイコン・MODカラーを一括初期化するボタン付き。設定画面はOKボタン1つで反映・閉じるを行い、MOD一覧欄をコンパクト化。IITC.statusbar API の無い旧ビルド (iOS版アプリ等) にもフォールバック対応。
    ...
    self.showSettings = function () {
        ...
        for (var i = 0; i < keys.length; i++) {
            ...
            rows += '<tr>' +
                ...
                '<td><input type="text" data-modkey="' + k + '" value="' + val + '" style="width:30px;"></td>' +
